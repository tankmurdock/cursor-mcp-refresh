...
class MCPServersProvider {
    ...
    async refreshEnabledServers() {
        const enabledServersList = this.servers.filter((server) => this.enabledServers.has(server.name));
        if (enabledServersList.length === 0) {
            vscode.window.showWarningMessage("No MCP servers are enabled. Use the server selector to enable servers.");
            return;
        }
        const refreshPromises = enabledServersList.map((server) => this.refreshServer(server.name));
        await Promise.all(refreshPromises);
        vscode.window.showInformationMessage(`Refreshed ${enabledServersList.length} enabled MCP servers`);
    }
    loadEnabledServers() {
        const config = vscode.workspace.getConfiguration("cursor-mcp-refresh");
        const enabledServersList = config.get("enabledServers", []);
        this.enabledServers = new Set(enabledServersList);
    }
    async saveEnabledServers() {
        const config = vscode.workspace.getConfiguration("cursor-mcp-refresh");
        await config.update("enabledServers", Array.from(this.enabledServers), vscode.ConfigurationTarget.Global);
    }
    async showServerSelector() {
        if (this.servers.length === 0) {
            vscode.window.showWarningMessage("No MCP servers found. Please check your mcp.json configuration.");
            return;
        }
        const items = this.servers.map((server) => ({
            label: server.name,
            description: `${server.config.command} ${server.config.args.join(" ")}`.substring(0, 60),
            detail: server.status === "running"
        ...
        if (selected) {
            ...
            vscode.window.showInformationMessage(`Updated enabled servers: ${Array.from(this.enabledServers).join(", ") || "None"}`);
            this.refresh();
        }
    }
    getEnabledServersStatus() {
        if (this.servers.length === 0) {
            return "No servers";
        }
        const enabledCount = this.servers.filter((server) => this.enabledServers.has(server.name)).length;
...
function activate(context) {
    ...
    const refreshEnabledCommand = vscode.commands.registerCommand("cursor-mcp-refresh.refreshEnabledServers", () => {
    ...
    const selectServersCommand = vscode.commands.registerCommand("cursor-mcp-refresh.selectServers", () => {
    ...
    context.subscriptions.push(treeView, refreshAllCommand, refreshEnabledCommand, refreshSingleCommand, selectServersCommand, statusBarItem);
